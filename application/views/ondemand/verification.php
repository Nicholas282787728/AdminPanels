                            ...
                            <tbody>
                                ...
                                <tr class="odd gradeX" >
									...
									<td>
										<?php  $id = $val['ind']; ?>
												<?php
												$query = $this->db->query("SELECT id from `uf_user_doc_store`  where seeStatus <> '0' AND id = '$id'"); 
			$result=$query->row();
			if(!empty($result)){
				?>
				<a data-toggle="modal" class="documentSendNotification" data-target="#myModal23" data-id="<?php echo $id; ?>">

					<input type="hidden" name="id" id="userid" value="<?php echo $this->session->userdata('id'); ?>">
...
<script type="text/javascript">
 ...
 var seeStatusId = jQuery(this).attr('data-id'); 
      ...
      jQuery.ajax({    //create an ajax request to display.php
        ...
        jQuery(".documentSendNotification").click(function() {  
                 
 var verficationId = jQuery(this).attr('data-id'); 
 ...
 // alert(userid);
      ...
      jQuery.ajax({    //create an ajax request to display.php
        type: "POST",
        url: "https://medconnex.net/demonil/panels/supermacdaddy/ondemand/verification",             
        data:{userid:userid,verficationId:verficationId},   //expect html to be returned                
        success: function(response){                    
         //location.reload();
