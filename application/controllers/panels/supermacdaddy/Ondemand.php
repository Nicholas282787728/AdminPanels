		...
		public  function service_update_document()
	{
		...
		//	$user_id=$this->db->insert_id();
				...
				foreach ($_FILES['upload']['name'] as $name => $value)
				{
					...
					$data_ins	= array('user_id'=>$user_id, 'document'=>$ImageName, 'document_name' => $arraydata[$name],'pendingUpgrades' => '1');
		...
		if(isset($_POST['userid'])){
			$data1 = array('seeStatus' =>'2');
			$this->db->where('id',$_POST['verficationId']);
			$this->db->update('uf_user_doc_store',$data1);

		//$documentSendNotification = array('userid' => '1','message' => 'testing verification documents','sender_id' => $_POST['userid']);
		 $documentSendNotification=array("user_id"=>$uid,"message"=>'Please provide me see again documents',"created_at"=>date('Y-m-d H:i:s'),'sender_id' => $_POST['userid'],'verficationId' => $_POST['verficationId']);
		   $this->db->insert('notification_history',$documentSendNotification);
		}
...
$data['alluser'] = $this->Ondemand_model->getUserDataForVerifacation();
