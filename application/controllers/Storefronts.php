...
// 		$data['title'] = 'Products';
// 		$data['file'] = 'storefronts/giftedProducts';
...
public function giftedProducts() {
		...
		if(!empty($_REQUEST['claimed'])){

		$userid = $this->session->userdata('id');
		$type = $this->input->post('claimed');

				
		$data['giftedAllProducts'] = $this->Store_model->giftedAllProducts($userid,$type);
	
	

	// else{
	// 	$this->session->set_flashdata('successmessage', 'Your '.$random_code.' Referral Code Is Not Exist');
	// 		redirect("storefronts/giftedProducts");
	// }
		...
		if ($this->session->userdata('adminlogin')) {
			...
			if (isset($_POST['createdticket'])) {
				...
				if (!empty($message_ticket) && $message_ticket != "<br>" && $_FILES["image"]["name"] != '') {
					...
					redirect("storefronts-support-tickets");
				} else {
	...
	public function notification() {
		if ($this->session->userdata('adminlogin')) {
			...
			echo '<li>
                            <a class="text-center brdr-0" href="' . base_url('storefronts-notifications') . '">
	...
	public function tasknotification() {
		if ($this->session->userdata('adminlogin')) {
			...
			echo '<li>
                            <a class="text-center brdr-0" href="' . base_url('storefronts-tasks') . '">
	...
	public function msgnotification() {
		if ($this->session->userdata('adminlogin')) {
			...
			echo '<li>
                            <a class="text-center brdr-0" href="' . base_url('storefronts-admin0') . '">
	...
	public function auth_user() {
		if ($this->session->userdata('adminlogin')) {
			...
			if (isset($_POST['enable'])) {
				...
				redirect("storefronts-auth-user");
			}
		...
		if ($this->session->userdata('adminlogin')) {
			...
			if (isset($_POST['updatedocument'])) {
				...
				if (!empty($_FILES["image"]["name"]) && in_array($ext, $allowed)) {
					...
					redirect("storefronts-signupdocuments");
				}
	...
	public function updateComp($value = '') {
		if (isset($_POST)) {
			...
			if ($result) {
				...
				redirect("storefronts-complimentaryAd");
			} else {
	...
	public function notifications() {
		...
		if (isset($_POST['update_read_status'])) {
			...
			redirect("storefronts-notifications");
		}
		$this->load->view('storefronts_templates', $data);
	}
	...
	public function verification() {
		...
		$data['alluser'] = $this->Store_model->get_Pendding_documents($vuid);
		$this->load->view('storefronts_templates', $data);
	}
	...
	public function subcategories() {
		...
		if (isset($_POST['save'])) {
			...
			redirect("storefronts-subcategories");
		}
		if (isset($_POST['enable'])) { //die('e here');
			$id = $_POST['enable'];
			$data = array("status" => '1');
	...
	public function liveOrders() {
		...
		if (isset($_POST['Order'])) {
			...
			redirect("storefronts-liveOrders");
		}
	     ...
	     public function composemail()
  {
	...
	public function history(){
		...
		$sql = "select * from sal_task where staff_id = '".$id."' limit 1";
		$query = $this->db->query($sql);
